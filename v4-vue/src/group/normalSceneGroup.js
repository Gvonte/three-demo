// normalScene场景的内容
import TWEEN from '../lib/tween.cjs';
...
export default async (gvonte) => {
  ...
  const normalSceneGroup = gvonte.createGroup(spriteText1, spriteText2, beam);

  const tween1 = new TWEEN.Tween(beam.material[1]).to({ opacity: 0 }, 1000).onComplete(() => {
    tween2.start();
  });
  const tween2 = new TWEEN.Tween(beam.material[1]).to({ opacity: 1 }, 1000).onComplete(() => {
    tween1.start();
  });
  tween1.start();
